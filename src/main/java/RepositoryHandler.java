...
public class RepositoryHandler implements InvocationHandler {
    ...
    private int handleSave(Object obj){
        ...
        try{
            PreparedStatement stmt = sqlConnection.getConn().prepareStatement(sql.toString());
    ...
    public int handleDeleteById(Class<?> clazz, Object idValue){
        ...
        sql.append(tableName).append(" WHERE ").append(idColumn).append(" = ?");
    ...
    public <T> T handleFindByCustom(Class<T> clazz, Object param, String paramName){
        ...
        sql.append(" WHERE ").append(paramName).append(" = ? ");
    ...
    private void checkAndThrow(Class<?> clazz){
        if(!clazz.isAnnotationPresent(Table.class)){
            throw new IllegalArgumentException("Class must have @annotations.Table annotation.");
        }
    }
    ...
    public <T> List<T> handleFindAll(Class<T> clazz){
        ...
        try{
            Statement stmt = sqlConnection.getConn().createStatement();
            ResultSet rs = stmt.executeQuery(sql.toString());
